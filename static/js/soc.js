...
function selectAlert(a){
  ...
  renderNotes(a.notes || []);
  renderEnrichment(a.enrichment || {});
}

function renderEnrichment(enrich) {
    if(!enrich || Object.keys(enrich).length === 0) {
        qs("enrichmentBox").style.display = "none";
        return;
    }
    
    qs("enrichmentBox").style.display = "block";
    if(enrich.error) {
         qs("enrichmentContent").innerHTML = `<div style="grid-column: span 2; color: var(--critical-red);">${enrich.error}</div>`;
         return;
    }
    
    qs("enrichmentContent").innerHTML = `
        <div><span style="color:var(--text-secondary);">Country:</span> ${enrich.country || '-'}</div>
        <div><span style="color:var(--text-secondary);">City:</span> ${enrich.city || '-'}</div>
        <div><span style="color:var(--text-secondary);">ISP:</span> ${enrich.isp || '-'}</div>
        <div><span style="color:var(--text-secondary);">ASN:</span> ${enrich.asn || '-'}</div>
    `;
}

async function enrichIP() {
    if(!selected || !selected.ip) return;
    
    // Show loading state
    qs("enrichmentBox").style.display = "block";
    qs("enrichmentContent").innerHTML = `<div style="grid-column: span 2; color:var(--text-secondary);"><i class="fas fa-spinner fa-spin"></i> Fetching Geographic OSINT...</div>`;
    
    try {
        const res = await fetch(`/api/soc/enrich/${selected.id}`, { method: "POST" });
        const data = await res.json();
        
        if(res.ok && data.enrichment) {
            selected.enrichment = data.enrichment;
            renderEnrichment(selected.enrichment);
        } else {
            qs("enrichmentContent").innerHTML = `<div style="grid-column: span 2; color: var(--critical-red);">Enrichment Failed: ${data.error || 'Unknown'}</div>`;
        }
    } catch(e) {
        qs("enrichmentContent").innerHTML = `<div style="grid-column: span 2; color: var(--critical-red);">Network API Error.</div>`;
    }
}
...
function exportJSON(){
  window.location.href = "/api/soc/export_report?format=json";
}
...
// NEW: Connect "Analyze with AI" to the new AI Companion page
window.analyzeAlertWithAI = function(alertData) {
  if(!alertData) return;
  
  // Store the payload in sessionStorage so the tutor page can pick it up
  sessionStorage.setItem('sentinel_tutor_context', JSON.stringify(alertData));
  
  // Redirect to the new tutor page
