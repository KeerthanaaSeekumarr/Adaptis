    ...
    try {
        ...
        if (tbody) {
            ...
            const filteredPackets = packets.filter(p => {
                ...
                if (selectedAttackType === 'THREATS') return p.severity.toLowerCase() !== 'low';
            ...
            tbody.innerHTML = filteredPackets.slice().reverse().slice(0, 50).map(p => {
                const isMalicious = p.severity.toLowerCase() !== 'low';
                ...
                let actionBtn = '';
                if (p.prevention_action === 'BLOCK_IP' && p.severity.toLowerCase() !== 'low') {
                ...
                } else if (p.prevention_action === 'RATE_LIMIT' && p.severity.toLowerCase() !== 'low') {
        ...
        if (totalPacketsElement && totalThreatsElement) {
            ...
            const threats = packets.filter(p => p.severity.toLowerCase() !== 'low');
...
socket.on('connect', () => {
    addLog(`System Connected [${new Date().toLocaleTimeString()}]`);
});

socket.on('new_packet', (packet) => {
    updatePackets(); // trigger a fast UI refresh whenever a threat comes in
});
