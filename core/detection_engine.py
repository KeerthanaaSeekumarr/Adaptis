...
def _load_external_signatures(dataset_2000_content, malicious_dataset_5000_content):
    ...
    for a in CORE_ATTACK_SIGNATURES:
         core_for_sampling.append({
            ...
            "success_criteria": a.get("success_criteria", ["ATTACK_SUCCESS", "COMPROMISE_INDICATED"])
        })
...
class DetectionEngine:
    ...
    def _generate_traffic(self, thread_id):
        ...
        while not self.stop_event.is_set():
            ...
            if random.random() < 0.2:
                ...
                src_ip = f"{random.choice([23, 45, 89, 103, 114, 203, 185, 8, 12, 175, 210, 110])}.{random.randint(1, 254)}.{random.randint(1, 254)}.{random.randint(1, 254)}"
            ...
            with self.lock:
                ...
                if hasattr(self, 'socketio') and severity.upper() != 'LOW':
                    self.socketio.emit('new_packet', packet)
    ...
    def generate_simulated_ipdr_data(self, start_ip_str, end_ip_str, count=50):
        try:
            ...
            threat_count = sum(1 for p in simulated_packets if p['severity'].upper() != 'LOW')
